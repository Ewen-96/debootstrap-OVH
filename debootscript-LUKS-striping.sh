...
prepare_disk_lvm() {
	...
	lvcreate -y --type striped -i "$STRIPE_COUNT" -I "$STRIPE_SIZE" -L 20G -n root_luks "$VG_NAME"
	lvcreate -y --type striped -i "$STRIPE_COUNT" -I "$STRIPE_SIZE" -L 8G  -n swap "$VG_NAME"
	...
	lvcreate -y --type striped -i "$STRIPE_COUNT" -I "$STRIPE_SIZE" -L 2G  -n boot "$VG_NAME"
